...
const ArticleCategory = ({ className, id, name, logo, articles, tags }: ArticleCategoryT) => {
  return (
    <article className={cn("rounded-box", className)}>
      <div className="bg-base-300 w-full h-28 pt-4 rounded-t-box relative">
        {/* rounded absolute element */}
        <div className="bg-base-200 w-1/2 h-4 absolute top-0 left-0 rounded-t-box sm:w-1/3"></div>
        {/* rounded absolute element */}
        <div className="bg-base-200 w-full h-24 px-6 rounded-tr-box relative">
          <Cards logo={logo} />
        </div>
      </div>
      ...
      <div className="bg-base-300 w-full flex flex-col p-6 rounded-b-box relative">
        ...
        <div className="row w-full mt-6">
          <p className="font-bold row leading-relaxed" title="تعداد مقالات">
            <DocumentDuplicateIcon className="icon" />
            <span className="mr-1.5">{articles.length}</span>
          </p>
          <p className="font-bold row leading-relaxed mr-3" title="تعداد هشتگ ها">
...
const Cards = ({ logo }: { logo: string }) => {
  return (
    <>
      <div className="bg-base-100 shadow-lg shadow-base-300 w-1/3 h-[4.5rem] p-3 rounded-t-box -rotate-6 absolute -bottom-2.5 left-6">
      ...
      <div className="bg-base-100 shadow-lg shadow-base-300 w-1/3 h-12 p-3 rounded-t-box absolute left-1/2 bottom-0 -translate-x-2/3" dir="ltr">
        <span className="bg-base-content/10 block w-1/3 h-1 mt-1 rounded-full"></span>
        <span className="bg-base-content/10 block w-full h-1 mt-1 rounded-full"></span>
        <span className="bg-base-content/10 block w-full h-1 mt-1 rounded-full"></span>
      </div>

      <div className="bg-base-100 shadow-lg shadow-base-300 w-1/2 min-w-[100px] h-20 p-3 rounded-t-box absolute bottom-0 right-6">
        <div className="row">
          <Image
            className="ring-4 ring-base-content/10 size-10 rounded-full sm:size-12"
            src={logo}
            width={48}
            height={48}
            alt={"لوگوی دسته بندی"}
